class BaseRunner {
      ...
      oReq.onload = function(ev) {
        ...
        if (oReq.readyState === 4) {
          if (oReq.status === 200) {
            ...
            reject(
              new Error(`Failed to load ${url} . Status: [${oReq.status}]`)
            )
          }
        }
      }
  ...
  loadModel = async (modelInfo, workload) => {
    if (
      this._bLoaded &&
      this._currentModelInfo.modelFile === modelInfo.modelFile
    ) {
    ...
    if (
      !modelPath.toLowerCase().startsWith('https://') &&
      !modelPath.toLowerCase().startsWith('http://')
    ) {
    ...
    if (this._currentModelInfo.labelsFile != null) {
      ...
      // for local workload test
      if (
        !labelPath.toLowerCase().startsWith('https://') &&
        !labelPath.toLowerCase().startsWith('http://')
      ) {
        if (typeof workload !== 'undefined') {
