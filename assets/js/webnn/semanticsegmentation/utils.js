class utils {
  ...
  initInputOutput = () => {
    ...
    this.inputTensor = [
      new typedArray(this.modelInfo.inputSize.reduce((a, b) => a * b))
    ]
  ...
  loadModel = async () => {
    ...
    this.init()
    let result = await loadModelAndLabels(
      this.modelInfo.modelFile,
      this.processHandle,
      this.modelInfo.labelsFile
    )
    ...
    switch (modelFile.split('.').pop()) {
      ...
      case 'bin':
        ...
        const networkText = await loadUrl(
          networkFile,
          false,
          this.processHandle,
